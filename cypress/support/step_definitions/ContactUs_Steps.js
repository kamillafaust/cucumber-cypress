...
When('I type a first name', () => {
  cy.get('[name="first_name"]').type('Kamilla');
});

When('I type a last name', () => {
  cy.get('[name="last_name"]').type('Faust');
});

When('I type a email address', () => {
  cy.get('[name="email"]').type('kamilla@email.com');
});

When('I type a comment', () => {
  cy.get('textarea[name="message"]').type('Adding a comment');
});
